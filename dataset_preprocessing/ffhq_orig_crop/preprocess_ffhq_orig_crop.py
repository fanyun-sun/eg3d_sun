...
if __name__ == '__main__':
    ...
    for i, filename in tqdm(enumerate(cameras), total=max_images):
        ...
        # img = center_crop_image(img, 700, 700)
        dataset["labels"].append([filename, label])
        os.makedirs(os.path.dirname(os.path.join(args.dest, filename)), exist_ok=True)
        img.save(os.path.join(args.dest, filename), quality="maximum", compress_level=COMPRESS_LEVEL)
        ...
        flipped_img.save(os.path.join(args.dest, flipped_filename), quality="maximum", compress_level=COMPRESS_LEVEL)
